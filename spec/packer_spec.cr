...
describe "MessagePack::Packer" do
  ...
  describe "bin 8 (0xC4)" do
    ...
    it_packs(Bytes[8, 231], UInt8[0xC4, 0x02] + UInt8[0x08, 0xE7], "\b\xE7")

    # valid byte sequence for UTF-8 ("好".bytes # => [0xE5, 0xA5, 0xBD])
    it_packs(Bytes[0xE5, 0xA5, 0xBD], UInt8[0xC4, 0x03] + UInt8[0xE5, 0xA5, 0xBD], "好")
  end
  describe "bin 16 (0xC5)" do
    # invalid byte sequence for UTF-8 (just a binary)
    it_packs(as_slice(UInt8[0x08, 0xE7] * 0x100), UInt8[0xC5, 0x02, 0x00] + UInt8[0x08, 0xE7] * 0x100, String.new(as_slice(UInt8[0x08, 0xE7] * 0x100)))
    # valid byte sequence for UTF-8 ("好".bytes # => [0xE5, 0xA5, 0xBD])
    it_packs(as_slice(UInt8[0xE5, 0xA5, 0xBD] * 0x100), UInt8[0xC5, 0x03, 0x00] + UInt8[0xE5, 0xA5, 0xBD] * 0x100, String.new(as_slice(UInt8[0xE5, 0xA5, 0xBD] * 0x100)))
  end
  describe "bin 32 (0xC6)" do
    # invalid byte sequence for UTF-8 (just a binary)
    it_packs(as_slice(UInt8[0x08, 0xE7] * 0x10000), UInt8[0xC6, 0x00, 0x02, 0x00, 0x00] + UInt8[0x08, 0xE7] * 0x10000, String.new(as_slice(UInt8[0x08, 0xE7] * 0x10000)))
