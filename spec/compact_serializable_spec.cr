...
@[MessagePack::CompactSerializable::Options(variables: 8)]
...
struct CMessagePackAttr8
  include MessagePack::CompactSerializable
  include MessagePack::CompactSerializable::Strict
...
describe "MessagePack mapping" do
  ...
  it "CMessagePackAttr1 raises when data shorter" do
    expect_raises(MessagePack::Error, "CompactSerializable(CMessagePackAttr1): expect array with 3 elements, but got 2 at 0") do
      CMessagePackAttr1.from_msgpack(Bytes[146, 1, 2])
    end
  end

  it "CMessagePackAttr1 raises when data not array" do
    expect_raises(MessagePack::Error, "CompactSerializable(CMessagePackAttr1): unpacker expect Array type but got nil at 0") do
      CMessagePackAttr1.from_msgpack(nil.to_msgpack)
    end
  end

  it "CMessagePackAttr1 raises when got nil for not nil variable" do
    expect_raises(MessagePack::Error, "unexpected token 'nil' expected INT at 3") do
